...
> ⚠️ **Quan trọng**: PHẢI mở qua `http://localhost:8000`, KHÔNG mở file `index.html` trực tiếp.
> Khi mở trực tiếp bằng `file://`, các API call (`/api/overview`, `/api/history`, ...) sẽ không hoạt động.
...
1. Mở `firmware/esp32_mqtt_client/esp32_mqtt_client.ino` bằng Arduino IDE
2. Cập nhật các biến cấu hình:
   ```cpp
   #define WIFI_SSID    "your_wifi_name"
   #define WIFI_PASS    "your_wifi_password"
   #define MQTT_HOST    "ip_of_your_server"
   #define DEVICE_ID    "esp32-001"
   ```
3. Cài thư viện Arduino: `PubSubClient`, `ArduinoJson` (v6)
4. Chọn board ESP32 và upload

Chi tiết xem thêm tại `firmware/README.md`.
